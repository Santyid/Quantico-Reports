...
export type SegmentoTab = 'activos' | 'inactivos' | 'todos';
...
@Component({
  ...
  templateUrl: './segmentos-cards.component.html',
  styleUrl: './segmentos-cards.component.scss'
})
export class SegmentosCardsComponent {
  ...
  get filteredSegmentos(): SegmentoCard[] {
    ...
    } else if (this.activeTab === 'inactivos') {
  ...
  get totalPages(): number {
    return Math.ceil(this.totalFiltered / this.itemsPerPage);
  }
  ...
  getTabCount(tab: SegmentoTab): number {
    if (tab === 'activos') return this.segmentos.filter(s => s.estado === 'Activo').length;
    if (tab === 'inactivos') return this.segmentos.filter(s => s.estado === 'Inactivo').length;
